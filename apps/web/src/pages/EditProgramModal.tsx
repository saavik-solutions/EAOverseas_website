...
const EditProgramModal: React.FC<EditProgramModalProps> = ({ isOpen, onClose, onUpdate, program }) => {
    ...
    useEffect(() => {
        if (program) {
            const intake = program.intakeTimeline[0];
            ...
            setFormData({
                ...
                tuitionFee: program.tuitionFee.replace('$', '').replace(' /yr', '').replace('£', '').replace(',', ''),
                description: program.description || '',
                academicBackground: program.academicBackground || '',
                gpaRequirement: program.gpaRequirement || '',
                intake_period: intake?.period.split(' ')[0] || 'September',
                intake_start: intake?.startDate || '',
                intake_end: intake?.endDate || '',
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        const updatedProgram = {
            ...
            tuitionFee: `${program.tuitionFee.includes('£') ? '£' : '$'}${formData.tuitionFee} /yr`,
                ...
                <div className="p-8 md:p-12">
                    <div className="flex flex-col gap-3 mb-10">
                        <div className="flex items-center gap-3">
                            ...
                            <span className="text-[10px] font-black text-blue-600 uppercase tracking-[0.2em] bg-blue-50 px-3 py-1 rounded-full border border-blue-100/50">Update Offering</span>
                        </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-8">
                        ...
                        <div className="pt-6">
                            <button
                                type="submit"
                                className="w-full py-4 rounded-xl bg-blue-600 text-white text-[11px] font-black uppercase tracking-widest hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center gap-2 shadow-xl shadow-blue-200"
                            >
                                Save Changes
