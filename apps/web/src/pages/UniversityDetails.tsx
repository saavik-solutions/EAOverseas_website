            ...
            */}
            ...
            <main className="flex-1 overflow-y-auto p-4 lg:p-8 scroll-smooth">
                <div className="max-w-[95%] mx-auto pb-10">
                    {/* BEGIN: HeroSection */}
                    <section className="mb-12" data-purpose="university-identity">
                        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                            <div>
                                <div className="flex items-center gap-3 mb-4">
                                    <span className="bg-blue-50 text-[#2b6cee] text-xs font-bold px-3 py-1 rounded-full uppercase tracking-wider">#{university.globalRanking} Global Ranking</span>
                                    <span className="bg-gray-100 text-gray-600 text-xs font-bold px-3 py-1 rounded-full uppercase tracking-wider">{university.type}</span>
                                </div>
                                <h1 className="text-4xl md:text-5xl font-extrabold text-gray-900 mb-2">{university.name}</h1>
                                <p className="text-lg text-gray-500 flex items-center gap-2 mb-4">
                    ...
                    {/* BEGIN: QuickSnapshot */}
                    <section className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-16" data-purpose="university-stats">
                        {/* Item 1 */}
                        <div className="bg-white p-5 rounded-[12px] shadow-[0_4px_20px_-2px_rgba(0,0,0,0.05)] border border-gray-50 flex flex-col items-center text-center">
                            <span className="text-2xl mb-2">🎓</span>
                            <span className="text-sm font-semibold text-gray-900">{university.coursesCount} Courses</span>
                            <span className="text-xs text-gray-400 mt-1">Popular Programs</span>
                        </div>
                        {/* Item 2 */}
                        <div className="bg-white p-5 rounded-[12px] shadow-[0_4px_20px_-2px_rgba(0,0,0,0.05)] border border-gray-50 flex flex-col items-center text-center">
                            <span className="text-2xl mb-2">💰</span>
                            <span className="text-sm font-semibold text-gray-900">{university.avgTuition} Avg Tuition</span>
                            <span className="text-xs text-gray-400 mt-1">Per Academic Year</span>
                        </div>
                        {/* Item 3 */}
                        <div className="bg-white p-5 rounded-[12px] shadow-[0_4px_20px_-2px_rgba(0,0,0,0.05)] border border-gray-50 flex flex-col items-center text-center">
                            <span className="text-2xl mb-2">🏠</span>
                            <span className="text-sm font-semibold text-gray-900">{university.livingExpense} Living</span>
                            <span className="text-xs text-gray-400 mt-1">Average per month</span>
                        </div>
                        {/* Item 4 */}
                        <div className="bg-white p-5 rounded-[12px] shadow-[0_4px_20px_-2px_rgba(0,0,0,0.05)] border border-gray-50 flex flex-col items-center text-center">
                            <span className="text-2xl mb-2">📅</span>
                            <span className="text-sm font-semibold text-gray-900">Available Intakes</span>
                            <span className="text-xs text-gray-400 mt-1">{university.intakes}</span>
                        </div>
                        {/* Item 5 */}
                        <div className="bg-white p-5 rounded-[12px] shadow-[0_4px_20px_-2px_rgba(0,0,0,0.05)] border border-gray-50 flex flex-col items-center text-center">
                            <span className="text-2xl mb-2">🕒</span>
                            <span className="text-sm font-semibold text-gray-900">{university.partTimeRights}</span>
                            <span className="text-xs text-gray-400 mt-1">Part-time Work Rights</span>
                        </div>
                    </section>
                    {/* END: QuickSnapshot */}
                            ...
                            <section data-purpose="admission-steps">
                                ...
                                <div className="space-y-4">
                                    {university.admissionSteps.map((step) => (
                                        <div key={step.step} className="flex items-center gap-5 p-5 bg-white border border-gray-100 rounded-[12px] transition-all hover:border-[#2b6cee]/20">
                                            <span className="bg-[#2b6cee15] text-[#2b6cee] w-10 h-10 flex items-center justify-center font-bold rounded-lg flex-shrink-0 text-lg">{step.step}</span>
                                            <div>
                    ...
                    <div className="mt-24 text-center pb-20">
                        <div className="inline-block p-12 bg-white rounded-[12px] border border-gray-100 shadow-[0_4px_20px_-2px_rgba(0,0,0,0.05)] max-w-2xl">
                            <h2 className="text-2xl font-bold mb-4">Ready to start your application?</h2>
                            <p className="text-gray-500 mb-8">Get the complete university brochure including detailed course modules and alumni insights.</p>
                            <button
                                onClick={handleDownload}
                                className="w-full sm:w-auto bg-[#2b6cee] text-white px-10 py-4 rounded-[12px] font-bold text-lg hover:bg-blue-700 transition-all flex items-center justify-center gap-3 shadow-lg shadow-[#2b6cee]/30 mx-auto"
                            >
                                ...
                                Download {university.name} Details (PDF)
                            </button>
                        </div>
                    </div>
                    {/* END: BottomAction */}
            ...
            {/* BEGIN: FooterSimple */}
            <footer className="bg-white border-t border-gray-100 py-8 shrink-0">
                <div className="max-w-7xl mx-auto px-4 text-center">
                    <p className="text-sm text-gray-400">© 2023 EAOverseas Education Platform. All rights reserved.</p>
                </div>
            </footer>
            {/* END: FooterSimple */}
        </div>
