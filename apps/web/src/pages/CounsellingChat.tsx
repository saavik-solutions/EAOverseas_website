                ...
                <div className="h-16 bg-white border-b border-gray-200 flex items-center justify-between px-6 shrink-0 z-10 shadow-sm">
                    <div className="flex items-center gap-3">
                        <div className="relative">
                            ...
                            ></div>
                            <div className="absolute bottom-0 right-0 size-3 bg-green-500 rounded-full border-2 border-white shadow-sm"></div>
                        </div>
                        <div className="flex flex-col">
                            <h2 className="text-sm font-bold text-gray-900 flex items-center gap-2">
                                Consulting with {selectedStudent.name}
                            </h2>
                            <p className="text-xs text-gray-500">Topic: {selectedStudent.topic}</p>
                        </div>
                    </div>
                    <div className="flex items-center gap-4">
                        <div className="flex items-center gap-2 px-3 py-1.5 bg-blue-50 rounded-lg text-blue-600 border border-blue-100">
                            <span className="material-symbols-outlined text-lg icon-filled animate-pulse">timer</span>
                            <span className="text-sm font-bold font-mono">{formatTime(timeLeft)} left</span>
                        </div>
                        <div className="h-6 w-px bg-gray-200 hidden sm:block"></div>
                        <button
                            onClick={() => setSelectedStudent(null)}
                            className="px-4 py-2 bg-white hover:bg-red-50 text-red-600 border border-red-200 rounded-lg text-sm font-medium transition-colors flex items-center gap-2 shadow-sm"
                        >
                    ...
                    <div className="flex-1 flex flex-col bg-[#f5f7f8] relative min-w-0">
                        <div className="flex-1 overflow-y-auto p-4 sm:p-6 flex flex-col gap-6 scroll-smooth">
                            <div className="flex justify-center my-2">
                                <span className="text-[10px] font-medium text-gray-500 bg-black/5 px-3 py-1 rounded-full uppercase tracking-wider">Today</span>
                            </div>
                            <div className="flex justify-center">
                                <span className="text-xs text-gray-500 italic">Session started at 10:00 AM</span>
                            </div>

                            {/* Student Message (Left) */}
                            <div className="flex gap-3 max-w-2xl self-start group">
                                <div
                                    className="size-8 rounded-full bg-cover bg-center shrink-0 mt-1 border border-gray-200"
                                    style={{ backgroundImage: `url('${selectedStudent.image}')` }}
                                ></div>
                                <div className="flex flex-col gap-1 max-w-[85%]">
                                    <div className="flex items-baseline gap-2">
                                        <span className="text-xs font-semibold text-gray-900">{selectedStudent.name}</span>
                                        <span className="text-[10px] text-gray-500">10:01 AM</span>
                                    </div>
                                    <div className="bg-white border border-gray-200 p-3.5 rounded-2xl rounded-tl-none text-sm text-gray-900 shadow-sm">
                                        <p className="leading-relaxed">Hi Dr. Elena! I've been reviewing the checklist for the Tier 4 visa. I'm a bit concerned about the financial proof documents.</p>
                                    </div>
                                </div>
                            </div>

                            {/* Counselor Message (Right - Authenticated User) */}
                            <div className="flex gap-3 max-w-2xl self-end flex-row-reverse group">
                                <div
                                ...
                                <div className="flex flex-col gap-1 items-end max-w-[85%]">
                                    <div className="flex items-baseline gap-2 flex-row-reverse">
                                        ...
                                        <span className="text-[10px] text-gray-500">10:03 AM</span>
                                    </div>
                                    <div className="bg-blue-600 text-white p-3.5 rounded-2xl rounded-tr-none text-sm shadow-md">
                                        <p className="leading-relaxed">Hello {selectedStudent.name.split(' ')[0]}. No need to worry, that's what we are here for. Could you clarify which part specifically is causing confusion?</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="flex gap-3 max-w-2xl self-start group">
                                ...
                                <div className="flex flex-col gap-1 max-w-[85%]">
                                    <div className="flex items-baseline gap-2">
                                        <span className="text-xs font-semibold text-gray-900">{selectedStudent.name}</span>
                                        <span className="text-[10px] text-gray-500">10:04 AM</span>
                                    </div>
                                    <div className="bg-white border border-gray-200 p-3.5 rounded-2xl rounded-tl-none text-sm text-gray-900 shadow-sm">
                                        <p className="leading-relaxed">It's about the "28-day rule" for bank statements. My parents transferred the funds last week. Does the 28 days start from when the money enters the account or from when I print the statement?</p>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="p-4 bg-white border-t border-gray-200">
                            <div className="bg-gray-50 p-2 rounded-xl border border-gray-200 focus-within:ring-2 focus-within:ring-blue-500/20 focus-within:border-blue-500/50 transition-all flex items-end gap-2 shadow-sm">
                                <button className="p-2.5 text-gray-500 hover:text-blue-600 hover:bg-white rounded-lg transition-all" title="Attach Document">
                                    <span className="material-symbols-outlined text-[20px]">attach_file</span>
                                </button>
                                <textarea
                                    className="flex-1 bg-transparent border-none focus:ring-0 resize-none max-h-32 py-2.5 text-sm text-gray-900 placeholder:text-gray-500 leading-relaxed"
                                    placeholder="Type your message here..."
                                    rows={1}
                                    value={message}
                                    onChange={(e) => setMessage(e.target.value)}
                                ></textarea>
                                <button className="p-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-colors shadow-sm flex items-center justify-center group" title="Send Message">
